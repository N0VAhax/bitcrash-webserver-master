...
exports.security = function(req, res) {
    ...
    if (!user.mfa_secret) {
        ...
        var qrUri = 'otpauth://totp/bitcrash:' + user.username + '?secret=' + user.mfa_potential_secret + '&issuer=bitcrash';
...
exports.enableMfa = function(req, res, next) {
    ...
    if (otp !== expected) {
        ...
        var qrUri = 'otpauth://totp/bitcrash:' + user.username + '?secret=' + secret + '&issuer=bitcrash';
        user.qr_svg = qr.imageSync(qrUri, {type: 'svg'});
        user.sig = sig;
